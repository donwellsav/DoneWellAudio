...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: '2026-04-19',
    changes: [
      { type: 'feat', description: '**Batch 25 audit fixes** — resolves the 3 P0 + 5 P1 + 3 P2 findings from' },
      { type: 'feat', description: '**UX clarity pass** — 18 string rewrites on high-stress surfaces where an' },
      { type: 'feat', description: '**Larsen Threshold canvas** — `docs/canvas/` design-philosophy artifact' },
      { type: 'feat', description: 'Light-theme severity colors were eyeballed, never WCAG-tested. The existing inline comments ("7.1:1 on white") were optimistic by 2 full stops.' },
      { type: 'feat', description: 'The custom tab bars were hand-rolled `<button>` rows rather than Radix Tabs, inheriting only focus rings, not roving tabindex / arrow keys.' },
      { type: 'feat', description: 'ML inference defensively copied the caller\'s features buffer on every call — correct for race safety, but allocation-per-frame was hidden.' },
      { type: 'feat', description: 'Copy was written one-string-at-a-time over many batches; no pass had tightened the hedged verbs and passive constructions across all surfaces at once.' },
      { type: 'feat', description: '`lib/dsp/constants/uiConstants.ts` — `VIZ_COLORS_LIGHT` darkened: `GROWING` → `#c2410c` (5.20:1), `WHISTLE` → `#0e7490` (5.28:1), `INSTRUMENT` → `#15803d` (5.00:1). Inline comments rewritten to reflect real ratios.' },
      { type: 'feat', description: '`lib/dsp/mlInference.ts` — Replaced per-frame `new Float32Array(features)` with a pre-allocated double-buffer (swap `pendingBuffer` ↔ `inflightBuffer`). Same public `predictCached()` semantics, zero allocations on the hot path.' },
      { type: 'feat', description: '`hooks/useTabKeyboardNav.ts` — new helper that attaches ArrowLeft/Right/Home/End navigation to any `role="tablist"` container.' },
      { type: 'feat', description: '`components/analyzer/DesktopLayout.tsx` — wired the hook into both tab bars with full `role="tablist"` / `role="tab"` / `aria-selected` / roving `tabIndex`. Notification dot got `dark:bg-amber-400`.' },
      { type: 'feat', description: '`components/analyzer/HeaderBarTransportControls.tsx` — `h-9 tablet:h-11` → `h-11` uniformly, so mobile now meets 44×44.' },
      { type: 'feat', description: '`components/ui/console-slider.tsx` — `aria-label` on the click-to-edit number input.' },
      { type: 'feat', description: '`components/analyzer/IssueCard.tsx` — severity icon got `role="img"` + `aria-label`; inner SVG `aria-hidden`.' },
      { type: 'feat', description: '`components/analyzer/AudioAnalyzerAlerts.tsx` — dark variants on the worker-error banner.' },
      { type: 'feat', description: '`components/analyzer/LandscapeSettingsSheet.tsx` — `max-h-[70vh]` → `max-h-[85vh]` so iPhone-landscape settings have room.' },
      { type: 'feat', description: '`app/globals.css` — `tab-track-item` transition scoped to paint-only props (skips layout recalc); `--fader-glow-locked` / `--fader-glow-cal` tokens added in both `:root` and `.light`.' },
      { type: 'feat', description: '`components/analyzer/DualFaderStrip.tsx` — hardcoded shadow RGBA → the new CSS vars, one-theme-per-token.' },
      { type: 'feat', description: '`lib/utils/__tests__/advisoryDisplay.test.ts` — pins updated for the new light colors + added `WHISTLE` / `INSTRUMENT` regression coverage.' },
      { type: 'feat', description: '`AudioAnalyzerAlerts.tsx` — `"DSP worker error - analysis may be degraded. Auto-recovering..."` → `"Analysis engine hiccup — restarting automatically."`' },
      { type: 'feat', description: '`lib/analyzer/audioAnalyzerErrorGuidance.ts` — dropped `"may"` hedge, dropped `"(tab backgrounded?)"` parenthetical.' },
      { type: 'feat', description: '`IssueCard.tsx` — 3 operator notes (whistle / clustered / repeat band) rewritten verb-first, redundant whistle sentence removed.' },
      { type: 'feat', description: '`RingOutWizardSections.tsx` — 7 strings across listening / detected / summary phases: active verbs, concrete button names (`"press ENGAGE to start"` not `"start analysis first"`), tightened heuristic footer.' },
      { type: 'feat', description: '`DataConsentDialog.tsx` — CTA → `"Share anonymous data"`; GDPR bullets converted to scannable `topic — detail` form; 6th bullet shortened.' },
      { type: 'feat', description: '`IssueCard.test.tsx` + `RingOutWizardSections.test.tsx` — 6 string-pinned assertions updated; one switched from regex to exact-string match because the new copy\'s "broad region" phrase now substring-matches both the operator note and the strategy-label badge.' },
      { type: 'feat', description: '`larsen_threshold.md` — 7-paragraph design philosophy.' },
      { type: 'feat', description: '`larsen_threshold.pdf` — 9×12 in single-page scientific plate (reportlab vector, ~122 KB).' },
      { type: 'feat', description: '`larsen_threshold.png` — 300 dpi raster, 2700×3600, ~910 KB.' },
      { type: 'feat', description: '`render_larsen.py` — reportlab + numpy generator.' },
      { type: 'feat', description: '`npx tsc --noEmit` — clean, 0 errors.' },
      { type: 'feat', description: '`pnpm test` — **1572 pass / 4 skip / 0 fail** across 119 test files, 59 s.' },
      { type: 'feat', description: '5 failures anticipated by the plan appeared on the first test run (string-pinned assertions). Fixed the tests one-for-one, re-ran to green.' },
      { type: 'feat', description: 'Regression coverage added for the three darkened light-mode colors.' },
      { type: 'feat', description: '`mlInference.test.ts` (~50 cases) passes unchanged — public API identical.' },
      { type: 'feat', description: 'Visual verification deferred — every affected surface is conditional (worker crash, mic permission error, clustered advisory, ring-out wizard, EU consent dialog) and hard to trigger without live input. Unit tests render each component with the triggering props and assert the new copy.' },
      { type: 'feat', description: '[ ] Light-mode IssueCard with GROWING / WHISTLE / INSTRUMENT severities — confirm card text reads with WCAG AA contrast' },
      { type: 'feat', description: '[ ] DesktopLayout tab bars — keyboard ArrowLeft/Right/Home/End cycles focus; screen reader announces "tab N of M, selected"' },
      { type: 'feat', description: '[ ] Mobile phone viewport — ENGAGE / STOP / PAUSE / CLEAR hit targets ≥ 44 × 44 px' },
      { type: 'feat', description: '[ ] Trigger worker crash in dev (throw in `dspWorker.ts`) — confirm the new alert copy' },
      { type: 'feat', description: '[ ] Block mic permission — confirm new mic-error guidance strings' },
      { type: 'feat', description: '[ ] Generate a clustered advisory — confirm new operator note on the IssueCard' },
      { type: 'feat', description: '[ ] Walk through the Ring-Out wizard (listening → detected → summary) — confirm the 7 new strings' },
      { type: 'feat', description: '[ ] Force-show GDPR consent (EU) — confirm scannable bullets and the `"Share anonymous data"` CTA' },
    ],
  },
  {
    version: '0.104.0',
