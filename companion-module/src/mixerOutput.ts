...
export class MixerOutput {
  ...
  async applyAdvisory(advisory: DwaAdvisory): Promise<void> {
  ...
  private async sendOsc(advisory: DwaAdvisory): Promise<void> {
  ...
  private async sendTcp(advisory: DwaAdvisory): Promise<void> {
    if (!this.config.mixerHost) return
  ...
  private getTcpSocket(): Promise<net.Socket> {
    return new Promise((resolve, reject) => {
      ...
      const socket = net.createConnection(
        { host: this.config.mixerHost, port: this.config.mixerPort, timeout: 3000 },
        () => {
